...
<html lang="en">
  ...
  <h1>List of Clients <small>Refresh</small></h1>
	</div>
...
if($rows>0) {
 ...
 $data_p = mysql_query("SELECT * FROM `client` ORDER BY id ASC LIMIT ".$max) or die(mysql_error()); 
 ...
 { 
   ...
   echo '<td style="width:10%;">'.$info['id'].'</td>';
     echo '<td style="width:20%;">'.$info['fname'].'</td>';
	  echo '<td style="width:20%;">'.$info['lname'].'</td>';
	  echo '<td style="width:20%;">'.$info['product'].'</td>';
	  echo '<td style="width:20%;">'.$info['dateRenew'].'</td>';
	  echo '<td style="width:10%;"><a href="remove.php?id='.$info['id'].'"><button type="button" class="btn btn-default btn-xs">
 ...
 echo " <a href='{$_SERVER['PHP_SELF']}?pagenum=$last'>".'<span class="label label-primary glyphicon glyphicon-step-forward">Last</span></a> ';
	 ...
	 echo '<div class="alert alert-danger">No clients to remove!</div>';
 }
 ?> 
